...
> У меня нет возможности реализовать функционал старост или преподавателей, т.к. не располагаю такими аккаунтами.
...
```shell
pip install mtuci_privae_api
```
```shell
uv add mtuci_privae_api
```
...
Библиотека ищет в корне файл `logging_config.yaml` с настройкой логирования. Если не находит, то использует базовые настройки.
